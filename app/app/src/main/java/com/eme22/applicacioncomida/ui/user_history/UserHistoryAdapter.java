...
public class UserHistoryAdapter extends RecyclerView.Adapter<UserHistoryAdapter.EditorFragmentViewHolder> {
    ...
    public void onBindViewHolder(@NonNull EditorFragmentViewHolder holder, int position) {
        ...
        holder.property3.setElegantTextHeight(true);
        holder.property3.setInputType(InputType.TYPE_TEXT_FLAG_MULTI_LINE);
        holder.property3.setSingleLine(false);

        holder.property3.setText(MessageFormat.format("Items:\n{0}", calculateItems(item.getCartItems())));
    ...
    private String calculateItems(ArrayList<CartItem> cartItems) {
        ...
        for (CartItem item: cartItems) {
            if (item.getItem() != null)
                sb.append(item.getItem().getName()).append(" x ").append(item.getCount()).append("\n");
        }
