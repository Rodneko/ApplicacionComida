...
public class CartViewModel extends ViewModel {
    ...
    private final MutableLiveData<BigDecimal> cartPrice = new MutableLiveData<>(new BigDecimal("0.00"));

    LiveData<BigDecimal> getCartPrice() {
        return cartPrice;
    }
    ...
    public void addItem(CartItem cartItem, User user) {
        ...
        cartPrice.setValue(cartPrice.getValue().add(getTotal(cartItem)));
    ...
    private BigDecimal getTotal(CartItem cartItem) {
        BigDecimal price = BigDecimal.valueOf(cartItem.getItem().getPrice());
        int count = Math.toIntExact(cartItem.getCount());

        BigDecimal discount = cartItem.getItem().getPromo() != null ? BigDecimal.valueOf(cartItem.getItem().getPromo().getDiscount()) : new BigDecimal(0);

        BigDecimal actualPrice = price.multiply(new BigDecimal(count));

        BigDecimal totalDiscount = price.multiply(discount);

        return actualPrice.subtract(totalDiscount);
    }


    public void deleteCartItem(CartItem cartItem) {

        cartPrice.setValue(cartPrice.getValue().subtract(getTotal(cartItem)));
        ...
        service.uploadCart(cart.getValue()).enqueue(new Callback<Cart>() {
            ...
            public void onResponse(Call<Cart> call, Response<Cart> response) {
                if (response.isSuccessful()) {
                    ...
                    cartPrice.setValue(new BigDecimal(0));
                }
            }
